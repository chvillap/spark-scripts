...
Xtrain = [(1.0, Vectors.dense([0.0, 1.1, 0.1])),
          (0.0, Vectors.dense([2.0, 1.0, -1.0])),
          (0.0, Vectors.dense([2.0, 1.3, 1.0])),
          (1.0, Vectors.dense([0.0, 1.2, -0.5]))]
fields = ["label", "features"]
training = spark.createDataFrame(Xtrain, fields)
...
# Prepare test data.
Xtest = [(1.0, Vectors.dense([-1.0, 1.5, 1.3])),
         (0.0, Vectors.dense([3.0, 2.0, -0.1])),
         (1.0, Vectors.dense([0.0, 2.2, -1.5]))]
test = spark.createDataFrame(Xtest, fields)

# Make predictions on test data using the Transformer.transform() method.
# LogisticRegression.transform will only use the 'features' column.
# Note that model2.transform() outputs a "myProbability" column instead of the
# usual 'probability' column since we renamed the lr.probabilityCol parameter
# previously.
